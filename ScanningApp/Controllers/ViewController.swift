    ...
    func isARsupported() -> Bool {
        guard ARObjectScanningConfiguration.isSupported, ARWorldTrackingConfiguration.isSupported else {
            ...
            let okAction = UIAlertAction(title: "Okay", style: .default, handler: { action in
                fatalError("ARKit is not available on this device. Please use iPhone 6s or higher!!")
            })
            
            self.showAlert(title: nil, message: "ARKit is not available on this device. Please use iPhone 6s or higher!!", actions: [okAction])
